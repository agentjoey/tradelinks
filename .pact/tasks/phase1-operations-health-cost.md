...
- Final rework implementation pin (all four are blocking):
  1. Replace `src/email/transactional.ts`'s module-level `Set` with a `PipelineRun`-backed delivery adapter. Use one stable ledger row per structured alert key, call `sendOpsAlert`, and only set `finishedAt` after the result is exactly `"sent"`; `"skipped"`, `"failed"`, and thrown delivery errors must leave the row retryable. `health-check.ts`, `briefing-batch.ts`, and `cost-report.ts` must call this same adapter with structured `code`, `subjectId`, and full UTC-hour bucket.
  2. `src/jobs/briefing-batch.ts` must no longer contain the Task-4-deferred production no-op, and the hard-cap key must use the exact structured identity rather than the literal `"HARD_CAP"`.
  3. `src/monitoring/cost.ts` must parse and validate `RAILWAY_PROJECTED_MONTHLY_COSTS_JSON` as a non-empty object of finite non-negative component USD values, return both the component breakdown and total, and throw on missing/malformed input. `cost-report.ts` must not catch this as zero and must persist both the breakdown and the decision.
  4. Remove process-local suppression state. Add a production reader for the latest completed cost-report `PipelineRun`; `collect-batch.ts` must invoke it and exclude only contracts whose readiness is `EXPERIMENTAL` when `experimental-demand` is suppressed. It must leave MONITORED/VERIFIED official collection selected. Export a durable `model-enrichment` decision reader for the Task-5 legacy-worker retirement. Tests must execute these production-shaped readers/consumers; fakes that reimplement persistence do not satisfy this pin.
- Final acceptance clarification after production-path review:
  - Replace every bare string `recordOperationalAlert(key)` API with a structured alert input containing `code`, `subjectId`, and the full UTC-hour bucket. Briefing absence must supply all three values; no parser may silently discard a bare `BRIEFING_ABSENT` value.
  - The durable delivery adapter itself must query its `PipelineRun` row and skip sending when that exact row is already finished. Do not rely on `health-check.ts` module state or test setup for production dedup. Test adapter behavior directly: first `"sent"` finishes, same-key retry does not send, and `"skipped"`/`"failed"`/throw remain unfinished and retryable.
  - Delete `latestCostDecision`, `persistDecision`, and every process-local fallback. Export the actual durable decision reader and the actual Experimental-source predicate used by `collect-batch.ts`; tests must inject only the Prisma boundary/result, call these exported production functions, and prove official sources remain selected. Add direct coverage for `readModelEnrichmentSuppressed`.
- CLI registration pin: `scripts/run-job.ts` must side-effect import both `src/jobs/health-check.ts` and `src/jobs/cost-report.ts`, just as the accepted Task 1–3 finite handlers are loaded. Add a credential-free registry/CLI-path test proving both registered definitions have `run` handlers; a generic `--dry-run` result from `HEALTH_DEFAULTS` is not evidence.
- Code-quality closure pin: the exported `evaluateOperationalHealth(now)` read API must never deliver alerts or write the alert ledger; only the finite health job owns delivery side effects. A collect invocation must load the durable cost decision once and reuse that decision for all source filtering without module-level cache. Refresh `task-4-report.md` to the actual final commits/files/test counts with no placeholder values.
- Null-path closure: after the one durable decision read, collection must not call a per-source fallback reader when the decision is absent. Export and directly test the exact pure predicate used by the production selection path for null, normal, and hard-cap decisions. Replace every `_pending_` or equivalent placeholder in the Task 4 report with the actual commit/evidence.
