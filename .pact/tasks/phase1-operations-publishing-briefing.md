...
- Final rework pin: reconciliation is by identity. Persist `attemptedIds` and `failedIds` in `metadata.publishSummary`; union attempted IDs across replays, remove successful retry IDs from the prior failed set, and add current failed IDs. Derive `attempted`, `failed`, and `succeeded` from those sets. Replaying the same sticky failure three times must remain attempted 3 / succeeded 2 / failed 1; a new failed draft in a later replay must remain visible as PARTIAL / exit 1.
...
- Final rework pin: briefing replay persists/reads real attempted/succeeded/failed counts, the handler slices qualified versions to its maximum even when the dependency over-returns, and the production publish summary merges with existing PipelineRun metadata instead of replacing sibling keys.
