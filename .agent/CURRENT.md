...
### 2026-08-05 晚：时效下限 + 清理超期草稿（`262d745`）

**Owner 反馈：数量无法人工审，Shopify Changelog 这类开发者向细节不该出现。** 判断成立，且我漏掉的第一件事不是相关性而是**时效**——A02 那个 feed 带整个存档，1561 个簇回溯到 **2018-08-03**，促成任务把八年前的功能公告当成当前情报。

**已修**：促成要求锚点发布于 **90 天内**（`PROMOTION_MAX_AGE_DAYS`）。这是产品对"变更"的定义而非性能限制：90 天与各 hub 已展示的 `changeCount90d` 同口径，也覆盖联邦公报一个案子的周期。时效判在**锚点**上——旧报道仍可作为佐证随新公告同行（背景，非主张），但官方源本身过期的簇无论积压多深都不促成。选择查询里重复了该下限，避免每个时隙的配额被历史耗尽。

```
合格积压   1625 → 140     此后约 40 条/月
```

**已清理存量**：删除 178 条超期草稿（单事务，按 EvidenceRecord → Version → Change 顺序）。删前核实全部为 DRAFT、非 current、未复核、无周报引用。删后 **122 条**（76 PLATFORM_POLICY + 46 REGULATORY，最早 2026-05-07），孤儿 change 0、孤儿证据 0，3720 个证据簇完好未动。公开面复检：feed 0 项、API `data` 0 条、sitemap 15 条、路由全 200。

### 未解决：相关性判别（需 owner 决定）

时效下限解决了**数量**，没解决**质量**。90 天内的 Shopify 76 条仍以功能发布为主；联邦 46 条也多是**工业品**反倾销案（拖车、钢丝绞线、液压缸、蘑菇），跨境消费品卖家同样不需要行动。

试过用六个 policy topic 当门槛，**不成立**：`Cart sharing on POS` 确实落不进任何 topic，但"墨西哥拖车反倾销"技术上正好落进 `IMPORT_CUSTOMS`。把"payout 币种变更"与"POS 购物车共享"分开、"消费品召回"与"液压缸关税"分开是**判断**，源契约未编码，关键词规则做不到，不应用关键词冒充判断。

三条路：
- **A（推荐）接 AI 相关性判别**——仓库本就照此设计（`classify.ts` 明确期待模型建议，`requiresReview`/confidence 机制为此存在）。fail closed：判不准就不促成。**需要 `DEEPSEEK_API_KEY` 加到 `tradelinks-canonicalize` 服务**（该服务当前只有 DB 变量；本地 `.env` 也无任何 AI 密钥，故我无法测试）。
- **B 把 A02 降为佐证源**——零成本零密钥，代价是 Shopify 覆盖归零；但 `platform:shopify-us` 的 knownGaps 本就写着"仅官方公开更新日志"，这是如实暴露已知缺口。联邦工业品噪音仍在。
- **C 人工审这 140 条**——owner 已明确不可行。

### 仍未闭合

- **`AMZ-ANNOUNCEMENTS` 是哑源**：每次抓取 HTTP 200 / `SUCCEEDED_EMPTY` / 0 条，解析器取不到内容。它是 Amazon hub 唯一的政策源，所以该 hub 即便渲染也不会有内容。独立缺陷，待查。
- **`collect-fast` 修复尚无生产铁证**：08:09 时隙 E02 成功，但它在修复部署前的 04:07 也成功过一次——那个非法 `&` 随文章滚出 feed 窗口而间歇出现。证据目前是本地对实时 feed 的实测（0 → 56 条）与单元测试覆盖；生产需再观察数日无 FETCH_ERROR。
- **Neon 免费档**：100 CU-h/月，按当前节奏月末 66–88；超限即挂起计算实例。已按 owner 选择只清理分支（8 → 5），未升级。
- **内容仍需人工审核**：`/changes` 与各 hub 在首批审核通过前保持空态。
