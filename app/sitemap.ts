...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const cutoverEnabled =
    process.env.PUBLIC_CUTOVER_ENABLED === "true" || process.env.PUBLIC_CUTOVER_ENABLED === "1";

  // Legacy daily notes 308 to /briefings after the cutover, so they stop being
  // crawl targets. Skipping the query entirely also spares the database a
  // 1,000-row read on every sitemap request.
  const notes = cutoverEnabled ? [] : await getPublishedNotes(1000).catch(() => []);
  ...
  // After the cutover every legacy route 308s and `/zh` redirects to English
  // (owner decision 1, English-only launch). Advertising a redirect in a
  // sitemap asks crawlers to fetch a URL that will not serve — and listing
  // `/zh` contradicts the launch decision outright. Only `/` survives.
  const cutover = cutoverEnabled;

  const paths: { p: string; cf: "hourly" | "daily"; pr: number }[] = cutover
    ? [{ p: "/", cf: "hourly", pr: 1 }]
    : [
        { p: "/", cf: "hourly", pr: 1 },
  ...
  // Pre-cutover: en (root) + zh (/zh) for each. Post-cutover: English only.
