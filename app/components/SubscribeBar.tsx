    ...
    <div className="fixed inset-x-0 bottom-0 z-40">
      <div className="mx-auto m-3 flex max-w-3xl items-center gap-3 rounded-xl border border-signal/40 bg-surface2/95 px-4 py-3 shadow-2xl shadow-black/50 backdrop-blur sm:m-4">
        ...
        {state !== "done" && (
          <form onSubmit={submit} className="flex shrink-0 items-center gap-2">
            <input
              ...
              className="w-36 rounded-md border border-signal/30 bg-surface px-2 py-1.5 text-[12px] text-paper placeholder:text-faint sm:w-48"
            />
            <button
              type="submit"
        ...
        <button
          onClick={dismiss}
          aria-label="Dismiss"
          className="shrink-0 px-1 text-lg leading-none text-faint transition-colors hover:text-paper"
        >
          ×
        </button>
      </div>
