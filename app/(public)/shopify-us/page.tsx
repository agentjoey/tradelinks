...
export default async function ShopifyUsHubPage() {
  const hub = await getHub(SLUG);
  if (!hub) notFound();

  return (
    <>
      <JsonLd data={breadcrumbJsonLd([{ name: "TradeLinks", path: "/" }, { name: "US Market", path: "/us" }, { name: hub.title, path: "/shopify-us" }])} />
      <nav aria-label="Breadcrumb" className="ticker mb-3 flex flex-wrap gap-2 text-label uppercase tracking-[0.08em] text-faint">
        ...
        <span>/</span>
        <Link href="/us" className="transition-colors duration-200 hover:text-signal">US Market</Link>
        <span>/</span>
      ...
      <Section title="Federal requirements that also apply" sub="Market-level rules with no platform tag" />
