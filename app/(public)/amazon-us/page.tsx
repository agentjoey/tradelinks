...
export default async function AmazonUsHubPage() {
  const hub = await getHub(SLUG);
  if (!hub) notFound();

  return (
    <>
      <JsonLd data={breadcrumbJsonLd([{ name: "TradeLinks", path: "/" }, { name: "US Market", path: "/us" }, { name: hub.title, path: "/amazon-us" }])} />
